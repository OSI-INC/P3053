/*
	...
	This program initializes the EEM and implements the main event loop. The EEM
	is a programmable TCP/IP server with an eight-bit parallel bus mounted on an
	MPCIE card. It provides an eight-bit parallel bus on the MPCIE connections,
	by which the EEM can control the host circuit board into which we plug the
	EEM. The EEM uses a PIC32MZ2048EFH microcontroller and a LAN8720A Ethernet
	physical layer to provide computing, memory, and networking. The EEM
	provides only one network connection: a wired 100-Base-T connection for
	which the socket must be provided by the host. It also provides a UART
	console interface that we can use to view debug printouts and interact with
	the EEM through its Command-Line Interface (CLI). This same CLI is available
	over Telnet as well, through the network connection. 
	
	This program is part of the P3053 repository. The repository contains
	hundreds files taken from Microchip's Harmony3 library. These Harmony3 files
	provide the driver for our Ethernet physical layer (LAN8720A), the TCP/IP
	stack, utilities like "ping", and provide access to the PIC32MZ's
	...
	The majority of the source code files in this repository are provided by
	Microchip. These retain their their Microchip copyright statements. The
	top-level files are provided by Open Source Instruments Inc. (OSI). These we
	distribute under the GNU General Public License (GPL). The GPL is far more
	restrictive than the Microchip license. Anyone incorporating GPL files into
	their own work must release the completed work under GPL as well.
