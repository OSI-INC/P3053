...
OUTPUT_FILE=${DISTDIR}/P3053.${IMAGE_TYPE}.${OUTPUT_SUFFIX}
else
...
CFLAGS += -ffunction-sections -fdata-sections -O1 -fno-common \
          ...
          -Werror -Wall
ASFLAGS += -Wa,--defsym=__MPLAB_BUILD=1,--gdwarf-2
...
ASFLAGS += -D__DEBUG \
	-D__MPLAB_DEBUGGER_ICD4=1 \
    -Wa,--defsym=__MPLAB_DEBUG=1,--defsym=__MPLAB_DEBUGGER_ICD4=1,--gdwarf-2
endif
...
$(OBJECTDIR)/%.o : %.c
	@mkdir -p $(dir $@)
	@echo Compiling $<
	$(MP_CC) $(MP_EXTRA_CC_PRE) -x c -c -mprocessor=$(MP_PROCESSOR_OPTION) \
		$(CFLAGS) -o $@ $<

LDFLAGS_COMMON = \
	-mprocessor=$(MP_PROCESSOR_OPTION) \
	-Wl,--defsym=__MPLAB_BUILD=1$(MP_EXTRA_LD_POST)$(MP_LINKER_FILE_OPTION) \
	-Wl,--defsym=_min_heap_size=64960 \
	-Wl,--gc-sections \
	-Wl,--no-code-in-dinit \
	-Wl,--no-dinit-in-serial-mem \
	-Wl,-Map="$(DISTDIR)/$(PROJECTNAME).$(IMAGE_TYPE).map" \
	-Wl,--memorysummary,$(DISTDIR)/memoryfile.xml \
	-mdfp="$(DFP_DIR)"
	
ifeq ($(TYPE_IMAGE),DEBUG_RUN)
LDFLAGS_EXTRA = -g -mdebugger -D__MPLAB_DEBUGGER_ICD4=1 \
...
LDFLAGS_EXTRA =
OUTPUT_FILE := $(DISTDIR)/P3053.$(IMAGE_TYPE).$(DEBUGGABLE_SUFFIX)
